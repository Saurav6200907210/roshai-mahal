      ...
      <div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 md:grid-cols-4">
        <div className="md:col-span-2">
          <div className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-full bg-gradient-gold font-display text-xl font-bold text-primary-foreground">R</span>
            <div>
              <div className="font-display text-xl text-gradient-gold">Rosahai Mahal</div>
              <div className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Teghra · Begusarai</div>
            </div>
          </div>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
            {[
              ["Home", "#home"], ["About", "#about"], ["Menu", "#menu"], ["Gallery", "#gallery"],
              ["Reviews", "#reviews"], ["FAQ", "#faq"], ["Contact", "#contact"],
            ].map(([l, h]) => (
              <li key={l}><a className="transition hover:text-primary" href={h}>{l}</a></li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-muted-foreground">
            <li className="flex gap-2"><MapPin size={16} className="mt-0.5 text-primary"/>{RESTAURANT.address}</li>
            <li className="flex gap-2"><Phone size={16} className="mt-0.5 text-primary"/><a href={`tel:${RESTAURANT.phoneRaw}`}>{RESTAURANT.phone}</a></li>
            <li className="flex gap-2"><Mail size={16} className="mt-0.5 text-primary"/><a href={`mailto:${RESTAURANT.email}`}>{RESTAURANT.email}</a></li>
          </ul>
        </div>
      </div>
